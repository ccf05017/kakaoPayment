...
- [X] 결제 전체 취소 기능
    ...
    - [ ] 부분 취소된 결제는 결제전액취소 불가능
    
- [X] 결제전액취소 기능 중 카드사 전송 시 금액 데이터에 소수점이 들어가는 것으로 추정. 확인할 것.
    - 전송할 때 정수 String으로 포맷되도록 변경

- [ ] 결제 부분 취소 기능
    - [ ] 결제전액취소 된 결제 이력은 취소 불가능
    - [ ] 결제전액취소 이력은 취소 불가능
    - [ ] 결제취소한 적 없는 결제 이력이나 부분 취소된 적 있는 결제 이력에 대해 결제부분취소 진행 가능(부가가치세 자동 계산)
        - [ ] 이전 부분결제취소 금액의 총합과 요청된 부분결체취소 금액의 합이 결제 이력의 결제 금액보다 크면 결제부분취소 불가능
            - 불변식: sum(이전 부분결제취소 금액들) + 요청된 부분결제취소 금액 < 결제 이력 결제 금액
        - [ ] 자동계산 된 부가가치세가 남은 부가가치세보다 많으면 남은 부가가치세 만큼만 취소해야 한다.
            - 불변식: 부가가치세 자동 계산액 > 기존 부가가치세 총합 ⇒ 진행
    - [ ] 결제취소한 적 없는 결제 이력이나 부분 취소된 적 있는 결제 이력에 대해 결제부분취소 진행 가능(부가가치세 수동 계산)
        - [ ] 이전 부분결제취소 금액의 총합과 요청된 부분결체취소 금액의 합이 결제 이력의 결제 금액보다 크면 결제부분취소 불가능
            - 불변식: sum(이전 부분결제취소 금액들) + 요청된 부분결제취소 금액 < 결제 이력 결제 금액
        - [ ] 이전 부분결제취소 부가가치세 총합과 요청된 부분결제취소 부가가치세의 합이 결제 이력의 부가가치세보다 크면 결제부분취소 불가능
            - 불변식: sum(이전 부분결제취소 부가가치세들) + 요청된 부분결제취소 부가가치세 < 결제 이력 부가가치세
